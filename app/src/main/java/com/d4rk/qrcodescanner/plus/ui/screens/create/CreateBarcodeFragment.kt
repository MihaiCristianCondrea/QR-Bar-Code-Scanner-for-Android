...
class CreateBarcodeFragment : Fragment() {
    ...
    private fun buildItems() : List<PreferenceListItem> {
        return listOf(
            PreferenceListItem.Category(R.string.qr_code) , PreferenceListItem.Action(
                action = PreferenceAction.Clipboard , titleRes = R.string.content_from_clipboard , iconRes = R.drawable.ic_copy
            ) , PreferenceListItem.Action(
                action = PreferenceAction.Text , titleRes = R.string.text , iconRes = R.drawable.ic_text
            ) , PreferenceListItem.Action(
                action = PreferenceAction.Url , titleRes = R.string.url , iconRes = R.drawable.ic_link
            ) , PreferenceListItem.Action(
                action = PreferenceAction.Wifi , titleRes = R.string.wifi , iconRes = R.drawable.ic_wifi
            ) , PreferenceListItem.Action(
                action = PreferenceAction.Location , titleRes = R.string.location , iconRes = R.drawable.ic_location
            ) , PreferenceListItem.Action(
                action = PreferenceAction.Contact , titleRes = R.string.contact_v_card , iconRes = R.drawable.ic_contact_white
            ) , PreferenceListItem.Action(
                action = PreferenceAction.MoreQrCodes , titleRes = R.string.more_qr_codes , iconRes = R.drawable.ic_qr_code_white , showOpenInNew = true
            ) , PreferenceListItem.Category(R.string.barcode) , PreferenceListItem.Action(
                action = PreferenceAction.AllBarcodes , titleRes = R.string.all_barcodes_codes , iconRes = R.drawable.ic_barcode , showOpenInNew = true
            )
        )
    }
    ...
    private class PreferenceItemsAdapter(
        ...
        override fun onBindViewHolder(holder : RecyclerView.ViewHolder , position : Int) {
            when (holder) {
                ...
                is ActionViewHolder -> holder.bind(
                    getItem(position) as PreferenceListItem.Action , isFirstItemInSection(position) , isLastItemInSection(position)
                )
            }
        }
        ...
        private class ActionViewHolder(
            private val binding : ItemPreferenceBinding , private val onActionClicked : (PreferenceAction) -> Unit
        ) : RecyclerView.ViewHolder(binding.root) {
            ...
            fun bind(item : PreferenceListItem.Action , first : Boolean , last : Boolean) {
                ...
                if (item.showOpenInNew) {
                    if (binding.widgetFrame.isEmpty()) {
                        LayoutInflater.from(binding.root.context).inflate(R.layout.item_preference_widget_open_in_new , binding.widgetFrame , true)
                    }
            ...
            private fun applyCorners(card : MaterialCardView , first : Boolean , last : Boolean) {
                ...
                val shapeBuilder : ShapeAppearanceModel.Builder =
                        card.shapeAppearanceModel.toBuilder().setTopLeftCorner(CornerFamily.ROUNDED , if (first) dp24 else dp4).setTopRightCorner(CornerFamily.ROUNDED , if (first) dp24 else dp4).setBottomLeftCorner(CornerFamily.ROUNDED , if (last) dp24 else dp4)
    ...
    private sealed interface PreferenceListItem {
        ...
        data class Action(
            val action : PreferenceAction , @StringRes val titleRes : Int , @StringRes val summaryRes : Int? = null , @DrawableRes val iconRes : Int , val showOpenInNew : Boolean = false
        ) : PreferenceListItem
    }
