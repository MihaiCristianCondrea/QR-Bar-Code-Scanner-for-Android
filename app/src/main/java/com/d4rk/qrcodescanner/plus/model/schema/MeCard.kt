...
import com.d4rk.qrcodescanner.plus.utils.extension.parseOrNull
import com.d4rk.qrcodescanner.plus.utils.extension.removePrefixIgnoreCase
import com.d4rk.qrcodescanner.plus.utils.extension.removeStartAll
...
data class MeCard(
    val firstName : String? = null , val lastName : String? = null , val nickname : String? = null , val phone : String? = null , val email : String? = null , val birthday : String? = null , val note : String? = null , val address : String? = null
) : Schema {
    companion object {
    ...
    override fun toFormattedText() : String {
        ...
        return listOf(
            "${firstName.orEmpty()} ${lastName.orEmpty()}" , nickname , formattedBirthday , phone , email , note , formattedAddress
    ...
    override fun toBarcodeText() : String {
        ...
        return StringBuilder().append(SCHEMA_PREFIX).appendIfNotNullOrBlank(NAME_PREFIX , fullName , PARAMETER_SEPARATOR).appendIfNotNullOrBlank(NICKNAME_PREFIX , nickname , PARAMETER_SEPARATOR).appendIfNotNullOrBlank(PHONE_PREFIX , phone , PARAMETER_SEPARATOR)
                .appendIfNotNullOrBlank(EMAIL_PREFIX , email , PARAMETER_SEPARATOR).appendIfNotNullOrBlank(BIRTHDAY_PREFIX , birthday , PARAMETER_SEPARATOR).appendIfNotNullOrBlank(NOTE_PREFIX , note , PARAMETER_SEPARATOR).appendIfNotNullOrBlank(ADDRESS_PREFIX , address , PARAMETER_SEPARATOR)
