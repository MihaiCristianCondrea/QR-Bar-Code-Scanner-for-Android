...
class ErrorDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState : Bundle?) : Dialog {
        ...
        return MaterialAlertDialogBuilder(requireActivity()).setTitle(R.string.error).setMessage(message).setIcon(R.drawable.ic_warning).setCancelable(false).setPositiveButton(android.R.string.ok) { _ , _ -> listener?.onErrorDialogPositiveButtonClicked() }.create()
    }
}